  ...
  stages {
    stage('Build') {
      ...
      steps {
        ...
        snPublishApp(credentialsId: "${CREDENTIALS}", appSysId: "${APPSYSID}", url: "${DEVENV_URL}", appVersion: "1.0.14")
      }
    }
    stage('Deploy to Test') {
      ...
      steps {
        snInstallApp(credentialsId: "${CREDENTIALS}", url: "${TESTENV_URL}", appSysId: "${APPSYSID}", appScope: "${APPSCOPE}", appVersion: "1.0.14")
      }
    }
  }
}
